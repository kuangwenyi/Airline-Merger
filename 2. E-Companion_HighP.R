# This script tests Hypothesis 3 - High Performers
...
pkgs = c(
  ...
  'dplyr','stringr','lme4','RColorBrewer','broom.mixed','here', 'TwoWayFEWeights', 
  'DIDmultiplegt')
...
# Excluding 6 low performers
Essay3_R = Essay3_R[!(Essay3_R$airline %in% c ("AMERICAN",  "DELTA" ,  
                                              "EXPRESSJET","UNITED","US","FRONTIER")),]

#### 1. Event Study -  High Performer ####
...
# with controls
stack_HighP <- feols(OPOR ~ .[indicatorStacked] + 
...
# unpack
coef_ES_HighP = broom::tidy(stack_HighP, conf.int = TRUE)
write.csv(coef_ES_HighP, file = "coef_ES_HighP.csv")

ES_HighP <- broom::tidy(stack_HighP, conf.int = TRUE)[1:24,] %>%
...
model_IE = did_multiplegt(Essay3_R, Y, G, T, D, controls = controls, 
                          dynamic = 0, 
                          placebo = 4,
                          brep = 5, # brep (bootstrap) to get CI
                          parallel = TRUE, 
                          cluster = 'Carriercode') 
